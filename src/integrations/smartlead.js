...
function isNegativeOrAutomatedText(value) {
  ...
  return [
    ...
    /\bauto[-\s]?reply\b/,
    /\bautomated\s+response\b/,
    /\baway\s+from\s+(the\s+)?office\b/,
    /\bvacation\b/,
    /\bwill\s+(respond|reply)\s+.*\b(return|back)\b/,
    /\bplease\s+type\s+your\s+reply\s+above\s+this\s+line\b/,
    /\b(?:ticket|case)\s+(?:has\s+been\s+)?(?:opened|created)\b/,
    /\bin\s+receipt\s+of\s+your\s+request\b/,
    /\bthank\s+you\s+for\s+(?:contacting|your\s+email|reaching\s+out)\b/,
    /\bnot\s+interested\b/,
